...
# GTK Theme
git clone -q --depth 1 https://github.com/vinceliuice/Orchis-theme || die "Failed to clone GTK theme"
cd Orchis-theme
yes | ./install.sh -l -c dark -t grey --tweaks black >/dev/null 2>&1 || die "GTK theme install failed"

# Icon Theme  
cd "$TEMP_DIR"
git clone -q --depth 1 https://github.com/vinceliuice/Colloid-icon-theme || die "Failed to clone icon theme"
